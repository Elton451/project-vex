import type { EndPoint, Point, Segment } from "@shared/types/Endpoint";
import { endpointCompare } from "./endpointCompare";
import { segmentInFront } from "./segmentInFrontOf";
import { lineIntersection } from "./lineIntersection";

const { cos, sin } = Math;

export type Triangle = [Point, Point];
...
const getTrianglePoints = (
  ...
  const p1 = origin;
  const p2: Point = {
    x: origin.x + cos(angle1),
    y: origin.y + sin(angle1),
  };
  const p3: Point = { x: 0, y: 0 };
  const p4: Point = { x: 0, y: 0 };
...
export const calculateVisibility = (origin: Point, endpoints: EndPoint[]) => {
  let openSegments = [];
  let output = [];
  let beginAngle = 0;
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    for (let i = 0; i < endpoints.length; i += 1) {
      ...
      if (endpoint.beginsSegment) {
        ...
        while (segment && segmentInFront(endpoint.segment, segment, origin)) {
          index += 1;
